...
SoftwareSerial btSerial(3,4);//블루투스모듈 tx 3번 rx 4번
...
//MSP 열거형
enum{HEAD1,HEAD2,HEAD3, DATASIZE, CMD, ROLL, PITCH, YAW, THROTTLE, AUX, CRC, PACKETSIZE};
uint8_t mspPacket[PACKETSIZE];
uint8_t send_msg[PACKETSIZE];
uint8_t last_msg[PACKETSIZE];
...
int emg_val=0;
int last_throttle=0;
void setMotionMsg(uint8_t send_msg[PACKETSIZE])
{
...
//  Serial.println(emg_val);
  send_msg[HEAD1]='$';
  send_msg[HEAD2]='M';
  send_msg[HEAD3]='<';
  send_msg[DATASIZE]=5;
  send_msg[CMD]=150;
  send_msg[ROLL]=125 + angle_temp_y;
  send_msg[PITCH]=125 - angle_temp_x;
  send_msg[YAW]=125;
  send_msg[THROTTLE]=last_throttle;
...
//    Serial.println("I'm here");
  if(emg_val>300)
  {
    send_msg[THROTTLE]=last_throttle+10;
    if(send_msg[THROTTLE]>240 || send_msg[THROTTLE]<10)
    {
      send_msg[THROTTLE]=250;
    }
  }
  else if(emg_val<300)
  {
    send_msg[THROTTLE]=last_throttle-10;
    if(send_msg[THROTTLE]<9 || send_msg[THROTTLE]>240)
    {
      send_msg[THROTTLE]=0;    
    }
  }
  ...
  last_throttle= send_msg[THROTTLE];
  send_msg[AUX]=85;

  if((send_msg[ROLL]<140) && (send_msg[ROLL]>105) )
  {
    send_msg[ROLL]=125;
  }
  if((send_msg[PITCH]<140) && (send_msg[PITCH]>105) )
  {
    send_msg[PITCH]=125;
  }

  send_msg[CRC]=send_msg[DATASIZE]^send_msg[CMD]^send_msg[ROLL]^send_msg[PITCH]^send_msg[YAW]^send_msg[THROTTLE]^send_msg[AUX];
}
void pauseThrottleMotionMsg(uint8_t send_msg[PACKETSIZE])
{
  send_msg[HEAD1]='$';
  send_msg[HEAD2]='M';
  send_msg[HEAD3]='<';
...
void setCalibrationMsg(uint8_t send_msg[PACKETSIZE])
{
  send_msg[HEAD1]='$';
  send_msg[HEAD2]='M';
  send_msg[HEAD3]='<';
...
void setUnlockMsg(uint8_t send_msg[PACKETSIZE])
{
  send_msg[HEAD1]='$';
  send_msg[HEAD2]='M';
  send_msg[HEAD3]='<';
...
void setEmergencyMsg(uint8_t send_msg[PACKETSIZE])
{
  send_msg[HEAD1]='$';
  send_msg[HEAD2]='M';
  send_msg[HEAD3]='<';
...
void printMspPacket()
{
  Serial.print((char)send_msg[0]);
  Serial.print((char)send_msg[1]);
  Serial.print((char)send_msg[2]);
  Serial.print(send_msg[3]);
  Serial.print(' ');
  Serial.print(send_msg[4]);
  Serial.print('\t');
  Serial.print("R:");
  Serial.print(send_msg[5]);
  Serial.print('\t');
  Serial.print("P:");
  Serial.print(send_msg[6]);
  Serial.print('\t');
  Serial.print("Y");
  Serial.print(send_msg[7]);
  Serial.print('\t');
  Serial.print("T:");
  Serial.print(send_msg[8]);
  Serial.print('\t');
  Serial.print(send_msg[9]);
  Serial.print(' ');  
  Serial.print(send_msg[10]);
  Serial.print('\n');  
}
//최종 각도 계산 함수
void final_angle_calculate()
{
  int error;
  double dT;
  accel_t_gyro_union accel_t_gyro;
  ...
  angle_temp_x = angle_x;
  angle_temp_y = angle_y;
}
int startMsgCount=0;
const int pause_btn=8;
const int emergency_btn=9;
...
void loop()
{
  ...
  final_angle_calculate();
//  Serial.print("angleX = ");Serial.println(angle_temp_x);
//  Serial.print("angleY = ");Serial.println(angle_temp_y);
//  Serial.print("pause_btn value is ");
//  Serial.println(digitalRead(pause_btn));
//  Serial.print("emergency_btn value is ");
//  Serial.println(digitalRead(emergency_btn));
  if(Serial.available())
  {
    delay(5);  
  ...
  if(startMsgCount==0 && digitalRead(pause_btn) && digitalRead(emergency_btn) )
  {
    setCalibrationMsg(send_msg);
    for(int i=0; i<PACKETSIZE; i++)
    {
      //Serial.println("hi 1");
      btSerial.write(send_msg[i]);
      //Serial.println("--------------------------------------");
      startMsgCount=1;
      delay(5);
    }
    setUnlockMsg(send_msg);
    for(int i=0; i<PACKETSIZE; i++)
    {
      //Serial.println("--------------------------------------");
      btSerial.write(send_msg[i]);
  ...
  else if(digitalRead(pause_btn) && digitalRead(emergency_btn))
  {
//    Serial.println("hi 2");
    setMotionMsg(send_msg);
    for(int i=0; i<PACKETSIZE; i++)
    {
      btSerial.write(send_msg[i]);
      delay(5);
  ...
  else if(!digitalRead(emergency_btn))
  {
//    Serial.println("hi emergency");
    setEmergencyMsg(send_msg);
    last_throttle=0;
    for(int i=0; i<PACKETSIZE; i++)
    {
      startMsgCount=0;
//      Serial.print("Throttle is ");
//      Serial.println(send_msg[THROTTLE]);
      btSerial.write(send_msg[i]);
      delay(5);
    }    
  }
  ...
  {
//    Serial.println("hi pause_state");
    pauseThrottleMotionMsg(send_msg);
   //send_msg[THROTTLE]=last_msg[THROTTLE];
    for(int i=0; i<PACKETSIZE; i++)
    {
//      Serial.print("Throttle is ");
//      Serial.println(send_msg[THROTTLE]);
